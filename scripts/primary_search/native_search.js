...
async function updateAllRecipes(collection){
	try {
		for (let i = 0; i < collection.length; i++) {
			searchRecipeForAllWords(collection[i]);
			eliminateOrRestoreRecipe(collection[i]);
		}
	} catch (error) {
		return false;
	}
...
async function searchRecipeForAllWords(recipe) {
	...
	for (let i = 0; i < words.length; i++) {
		let result = searchRecipeForWord(words[i], recipeTitle, recipeIngredients, recipeDescription);
		results += `${result} `;
	}
...
const hideSearchEliminatedRecipes = async () => {
	...
	if(updateComplete){
		...
		for(let i = 0; i < eliminatedList.length; i++){
			eliminatedList[i].classList.replace("found", "notfound");
		}
	}
}
...
const displayRestoredRecipes = async () => {
	...
	if(updateComplete){
		...
		for(let i = 0; i < hiddenList.length; i++){
			if(!hiddenList[i].classList.contains("tagEliminated") && !hiddenList[i].classList.contains("searchEliminated")){
				hiddenList[i].classList.replace("notfound", "found");
			};
		}
	}
}
